...
const COMMON_ITEMS = ["Keys", "Door Lock", "Windows", "Lights", "Air Conditioner", "Fan", "Bed", "Mattress", "Wardrobe", "Desk", "Chair", "Bathroom Fixtures"];
...
export function CheckInForm({ tenantId }: CheckInFormProps) {
  ...
  const { data: checkInItems, refetch } = api.tenant.get.useQuery(
    { id: tenantId },
    {
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <div className="mt-4 space-y-4">
          ...
          {checkInItems?.length === 0 && <p className="text-center text-gray-500">No check-in items added yet.</p>}
        </div>
      </div>
    </div>
  );
